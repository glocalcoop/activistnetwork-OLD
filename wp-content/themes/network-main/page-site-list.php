		...
		<main role="main">
			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'clearfix' ); ?> itemscope itemtype="http://schema.org/BlogPosting">
				...
				<section class="directory-content" itemprop="articleBody" rel="main">
					...
					if(function_exists('glocal_site_categories')) {
						$categories = glocal_site_categories();
					} else {
						echo 'glocal_site_categories() not available.';
					}

					?>

					<ul class="filter js-menu">
						<li data-filter="*" class="is-on">All Categories</li>

						<?php
						foreach($categories as $category) {
							if(count($category['sites']) > 0) { ?>

								<li data-filter=".network-<?php echo $category['slug'] ?>"><?php echo $category['name'] ?></li>
					...
					<ul class="sites-list view-grid" id="isotope">
						...
						foreach ($sites as $site) {
							...
							$category_slug = preg_replace('/\s+/','-', strtolower($site_category));
							...
							<li class="isomote id-<?php echo $site_id; ?> site-<?php echo $site_slug; ?> network-<?php echo $category_slug; ?>" data-id="<?php echo $site_id ?>" data-slug="<?php echo $site_slug ?>" data-posts="<?php echo $site_details->post_count; ?>">
